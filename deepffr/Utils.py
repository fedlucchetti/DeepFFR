...
class Utils():
    ...
    def cluster_gate(self,envelope,diff_frequency):
        X = np.zeros([self.Nt,2])
        id = np.where(envelope>0.1)[0]
        diff_frequency[id]=0
        diff_frequency[0:100]=0
        for idx,x in enumerate(envelope):
            X[idx,0] = diff_frequency[idx]
            X[idx,1] = -1
    ...
    def instantaneous_diffphase(self,signal,frequency):
        phase                     = self.instantaneous_phase(signal)
        diff_phase                = np.absolute(phase - 2*np.pi*frequency*self.t)
        return diff_phase

    def instantaneous_diff_frequency(self,signal,frequency):
        diff_frequency = np.zeros(self.Nt)
        diff_phase     = self.instantaneous_diffphase(signal,frequency)
        # print(diff_phase.shape)
        diff_frequency[0:self.Nt-1] = np.diff(diff_phase)
        # diff_frequency[diff_frequency>1] = 1
        # diff_frequency[diff_frequency<1] = -1
        return diff_frequency
    ...
    def insilico_EFR(self,args):
        ...
        # create gating/envelope fundiff_phasection
        ...
        # gaussian                                                  = signal.gaussian(int(10/1000/self.dt), std=30)
...
if __name__ == "__main__":
    ...
    diff_frequency            = utils.instantaneous_diff_frequency(signal,frequency)
    # diff_frequency            = diff_frequency/max(diff_frequency)
    # diff_frequency            = diff_frequency/min(diff_frequency)
    features                      = utils.cluster_gate(envelope,diff_frequency)
    ...
    plt.subplot(111)
    # plt.plot(utils.t,sinusoid,label='EFR')
    gate = envelope*np.sin(diff_frequency)
    # plt.plot(utils.t,envelope*np.cos(diff_frequency),'b--',label='cos')
    plt.plot(utils.t,envelope,'k-',label='envelope')
    plt.plot(utils.t,gate/max(gate),'b--',label='sin')
    # plt.plot(utils.t,gate,'r--',label='gate')
    # plt.plot(utils.t,gate,'r--',label='gate')
    plt.plot([ton/fs,ton/fs],[0,1],'k',label=("$t_{ON} & t_{Off}$"))
    plt.plot([toff/fs,toff/fs],[0,1],'k')
    plt.legend()
    plt.title('In silico EFR  f = ' + str(frequency) + ' Hz   ' + 'SNR = ' + str(SNR) +' dB' )
    plt.xlabel("Time [s]")
    plt.ylabel("A.U")
